...
{
#ifdef YASLI_BIN_ARCHIVE_CHECK_EMPTY_NAME_MIX
  BlockType type = !strlen(name) ? POD : NON_POD;
  YASLI_ASSERT(!(type == NON_POD && blockTypes_.size() > 1 && *(blockTypes_.end() - 2) == POD), "Parent empty name node with non-empty name children is dangerous for BinArchives");
  YASLI_ASSERT(blockTypes_.back() == UNDEFINED || blockTypes_.back() == type, "Mixing empty and non-empty names is dangerous for BinArchives");
	blockTypes_.back() = type;
	blockTypes_.push_back(UNDEFINED);
#endif
...
bool BinOArchive::operator()(ContainerInterface& ser, const char* name, const char* label)
{
  ...
  if (size > 0) {
    int elementSize = (int)ser.elementType().sizeOf();
    if (elementSize > YASLI_BIN_ARCHIVE_CONTAINER_POD_SIZE && strlen(name)) {
      int i = 0;
      do {
        char buffer[16];
#ifdef _MSC_VER
...
bool BinIArchive::operator()(ContainerInterface& ser, const char* name, const char* label)
{
  ...
  ser.resize(size);
  if (size > 0) {
    int elementSize = ser.elementType().sizeOf();
    if (elementSize > YASLI_BIN_ARCHIVE_CONTAINER_POD_SIZE && strlen(name)) {
...
bool BinIArchive::Block::get(const char* name, Block& block)
{
  ...
  if (!disableCheck_) {
    ...
    if (i != hashMap_.end() && i->second != name)
      YASLI_ASSERT(0, "BinArchive hash colliding: %s - %s - %i", i->second.c_str(), name, hashName);
